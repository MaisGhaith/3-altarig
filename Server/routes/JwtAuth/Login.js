...
router.post('/login', (req, res) => {
    ...
    pool.query(sql, [user_email], async (error, results) => {
        if (error) {
        ...
        if (user.flag === false) {
            return res.send("Your email is not verified, please verify it");
        }
        ...
        if (user.deleted === true) {
            return res.send('Your account was deleted or not verified, please try again with another email');
        }
